...
void getGrad_ffff(_gpu_type gpu)
{
    ...
    gpuMemcpy(dev_uint8_buffer, &trans, sizeof(uint8_t) * ERI_GRAD_FFFF_SMEM_UINT8_SIZE, cudaMemcpyHostToDevice);
    gpuMemcpy(dev_uint8_ptr_buffer, uint8_ptr_buffer, sizeof(uint8_t *) * ERI_GRAD_FFFF_SMEM_UINT8_PTR_SIZE * ERI_GRAD_FFFF_TPB, cudaMemcpyHostToDevice);
    gpuMemcpy(dev_uint32_buffer, uint32_buffer, sizeof(uint32_t) * ERI_GRAD_FFFF_SMEM_UINT32_SIZE * ERI_GRAD_FFFF_TPB, cudaMemcpyHostToDevice);
    gpuMemcpy(dev_uint32_ptr_buffer, uint32_ptr_buffer, sizeof(uint32_t *) * ERI_GRAD_FFFF_SMEM_UINT32_PTR_SIZE * ERI_GRAD_FFFF_TPB, cudaMemcpyHostToDevice);
    gpuMemcpy(dev_dbl_buffer, dbl_buffer, sizeof(QUICKDouble) * ERI_GRAD_FFFF_SMEM_DBL_SIZE * ERI_GRAD_FFFF_TPB, cudaMemcpyHostToDevice);
    gpuMemcpy(dev_dbl_ptr_buffer, dbl_ptr_buffer, sizeof(QUICKDouble *) * ERI_GRAD_FFFF_SMEM_DBL_PTR_SIZE * ERI_GRAD_FFFF_TPB, cudaMemcpyHostToDevice);
    gpuMemcpy(dev_int2_ptr_buffer, int2_ptr_buffer, sizeof(int2 *) * ERI_GRAD_FFFF_SMEM_INT2_PTR_SIZE * ERI_GRAD_FFFF_TPB, cudaMemcpyHostToDevice);
    gpuMemcpy(dev_char_ptr_buffer, char_ptr_buffer, sizeof(unsigned char *) * ERI_GRAD_FFFF_SMEM_CHAR_PTR_SIZE * ERI_GRAD_FFFF_TPB, cudaMemcpyHostToDevice);
    gpuMemcpy(dev_grad_ptr_buffer, grad_ptr_buffer, sizeof(QUICKAtomicType *) * ERI_GRAD_FFFF_SMEM_PTR_SIZE * ERI_GRAD_FFFF_TPB, cudaMemcpyHostToDevice);
