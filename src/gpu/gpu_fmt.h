#if !defined(__QUICK_GPU_FMT_H_)
#define __QUICK_GPU_FMT_H_

#include <assert.h>


/* Incrementally evaluate Boys functions using the recurrence relation method
 *
 * Boys function: F_{n}(x) = \int_{t = 0}^{t = 1} t^{2n} * e^{-xt^2} dt
 *         where: n is a positive integer, 0 <= n <= 24
 *                x is a non-negative real number
 *
 * inputs:
 *   n: as defined above (n)
 *   X: as defined above (x)
 * outputs:
 *   YVerticalTemp: Boys values (stored according to indexing macro VY)
 *
 * Reference: GPU Acceleration of the Boys Function Evaluation in
 *   Computational Quantum Chemistry. Tsuji et. al.
 *   DOI: 10.1002/cpe.8328
 */
__device__ static inline void FMT_NAME(const int n, const QUICKDouble X,
        QUICKDouble * const YVerticalTemp) {
    int j;

    assert(X >= 0.0);
    assert(n >= 0);

    if (X == 0.0) {
        for (j = 0; j <= n; ++j) {
	    VY(0, 0, j) = __drcp_rn(2 * j + 1);
        }
    } else if (X > 0.352906 * n + 0.015533) {
        double ex;
        double b;
        double X_inv2 = __drcp_rn(2 * X);

        /* recurrence relation method */
        if (X < 0.064049 * n + 28.487432) {
            ex = -exp(-X);
            b = 0.5 * __dsqrt_rn(M_PI / X) * erf(__dsqrt_rn(X));
        } 
        /* semi-indefinite interval method,
         * which takes e^{-X} \approx 0 and erf(\sqrt{X}) \approx 1
         * for sufficiently large n and X */
        else {
            ex = 0.0;
            b = 0.5 * __dsqrt_rn(M_PI / X);
        }

        VY(0, 0, 0) = b;
        for (j = 1; j <= n; j++) {
	    b = __fma_rn((2 * j - 1), b, ex) * X_inv2;
	    VY(0, 0, j) = b;
        }
    }
    /* fallback to older spline approximation method
     *
     * TODO: implement gridded Taylor expansion method
     *       with precomputed lookup table to replace this
     */
    else {
        const QUICKDouble XINV = 1.0 / X;
        const QUICKDouble E = exp(-X);
        QUICKDouble WW1;

        if (X > 5.0) {
            WW1 = sqrt(PIE4 * XINV);
        } else {
            WW1 = E;
        }

        if (X > 33.0) {

        } else if (X <= 33.0 && X > 15.0) {
            WW1 += ((1.9623264149430E-01 * XINV
                        - 4.9695241464490E-01) * XINV
                    - 6.0156581186481E-05) * E;
        } else if (X > 10.0) {
            WW1 += (((-1.8784686463512E-01 * XINV
                            + 2.2991849164985E-01) * XINV
                        - 4.9893752514047E-01) * XINV
                    - 2.1916512131607E-05) * E;
        } else if (X > 5.0) {
            WW1 += ((((((4.6897511375022E-01 * XINV - 6.9955602298985E-01) * XINV
                                    + 5.3689283271887E-01) * XINV
                                - 3.2883030418398E-01) * XINV
                            + 2.4645596956002E-01) * XINV
                        - 4.9984072848436E-01) * XINV
                    - 3.1501078774085E-06) * E;
        } else if (X > 3.0) {
            QUICKDouble Y = X - 4.0;
            WW1 += (X + X) * (((((((((((-2.62453564772299E-11 * Y + 3.24031041623823E-10) * Y
                                                    - 3.614965656163E-09) * Y + 3.760256799971E-08) * Y
                                            - 3.553558319675E-07) * Y + 3.022556449731E-06) * Y
                                    - 2.290098979647E-05) * Y + 1.526537461148E-04) * Y
                            - 8.81947375894379E-04) * Y + 4.33207949514611E-03) * Y
                    - 1.75257821619926E-02) * Y + 5.28406320615584E-02);
        } else if (X > 1.0) {
            QUICKDouble Y = X - 2.0;
            WW1 += (X + X) * (((((((((((-1.61702782425558E-10 * Y + 1.96215250865776E-09) * Y
                                                    - 2.14234468198419E-08) * Y + 2.17216556336318E-07) * Y
                                            - 1.98850171329371E-06) * Y + 1.62429321438911E-05) * Y
                                    - 1.16740298039895E-04) * Y + 7.24888732052332E-04) * Y
                            - 3.79490003707156E-03) * Y + 1.61723488664661E-02) * Y
                    - 5.29428148329736E-02) * Y + 1.15702180856167E-01);
        } else if (X > 1.0E-1 || (X > 1.0E-4 && n < 4)) {
            WW1 += (X + X) * (((((((((-8.36313918003957E-08 * X + 1.21222603512827E-06) * X
                                            - 1.15662609053481E-05) * X + 9.25197374512647E-05) * X
                                    - 6.40994113129432E-04) * X + 3.78787044215009E-03) * X
                            - 1.85185172458485E-02) * X + 7.14285713298222E-02) * X
                    - 1.99999999997023E-01) * X + 3.33333333333318E-01);
        } else {
            WW1 = (1.0 - X) / (2.0 * n + 1.0);
        }

        if (X > 1.0E-1 || (X > 1.0E-4 && n < 4)) {
            VY(0, 0, 0) = WW1;
            for (int m = 1; m <= n; m++) {
                VY(0, 0, m) = (((2.0 * m - 1.0) * VY(0, 0, m - 1)) - E) * 0.5 * XINV;
            }
        } else {
            VY(0, 0, n) = WW1;
            for (int m = n - 1; m >= 0; m--) {
                VY(0, 0, m) = (2.0 * X * VY(0, 0, m + 1) + E) / (m * 2.0 + 1.0);
            }
        }
    }
}


/* Incrementally evaluate Boys functions values using the recurrence relation method
 * where the starting value in the recurrence is approximated via fitted polynomial
 * spline functions
 *
 * Boys function: F_{n}(x) = \int_{t = 0}^{t = 1} t^{2n} * e^{-xt^2} dt
 *         where: n is a positive integer, 0 <= n <= 24
 *                x is a non-negative real number
 *
 * inputs:
 *   n: as defined above (n)
 *   X: as defined above (x)
 * outputs:
 *   YVerticalTemp: Boys values (stored according to indexing macro VY)
 */
//__device__ static inline void FMT_NAME(const int n, const QUICKDouble X,
//        QUICKDouble * const YVerticalTemp) {
//    const QUICKDouble XINV = 1.0 / X;
//    const QUICKDouble E = exp(-X);
//    QUICKDouble WW1;
//
//    if (X > 5.0) {
//        WW1 = sqrt(PIE4 * XINV);
//    } else {
//        WW1 = E;
//    }
//
//    if (X > 33.0) {
//
//    } else if (X <= 33.0 && X > 15.0) {
//        WW1 += ((1.9623264149430E-01 * XINV
//                    - 4.9695241464490E-01) * XINV
//                - 6.0156581186481E-05) * E;
//    } else if (X > 10.0) {
//        WW1 += (((-1.8784686463512E-01 * XINV
//                        + 2.2991849164985E-01) * XINV
//                    - 4.9893752514047E-01) * XINV
//                - 2.1916512131607E-05) * E;
//    } else if (X > 5.0) {
//        WW1 += ((((((4.6897511375022E-01 * XINV - 6.9955602298985E-01) * XINV
//                                + 5.3689283271887E-01) * XINV
//                            - 3.2883030418398E-01) * XINV
//                        + 2.4645596956002E-01) * XINV
//                    - 4.9984072848436E-01) * XINV
//                - 3.1501078774085E-06) * E;
//    } else if (X > 3.0) {
//        QUICKDouble Y = X - 4.0;
//        WW1 += (X + X) * (((((((((((-2.62453564772299E-11 * Y + 3.24031041623823E-10) * Y
//                                                - 3.614965656163E-09) * Y + 3.760256799971E-08) * Y
//                                        - 3.553558319675E-07) * Y + 3.022556449731E-06) * Y
//                                - 2.290098979647E-05) * Y + 1.526537461148E-04) * Y
//                        - 8.81947375894379E-04) * Y + 4.33207949514611E-03) * Y
//                - 1.75257821619926E-02) * Y + 5.28406320615584E-02);
//    } else if (X > 1.0) {
//        QUICKDouble Y = X - 2.0;
//        WW1 += (X + X) * (((((((((((-1.61702782425558E-10 * Y + 1.96215250865776E-09) * Y
//                                                - 2.14234468198419E-08) * Y + 2.17216556336318E-07) * Y
//                                        - 1.98850171329371E-06) * Y + 1.62429321438911E-05) * Y
//                                - 1.16740298039895E-04) * Y + 7.24888732052332E-04) * Y
//                        - 3.79490003707156E-03) * Y + 1.61723488664661E-02) * Y
//                - 5.29428148329736E-02) * Y + 1.15702180856167E-01);
//    } else if (X > 1.0E-1 || (X > 1.0E-4 && n < 4)) {
//        WW1 += (X + X) * (((((((((-8.36313918003957E-08 * X + 1.21222603512827E-06) * X
//                                        - 1.15662609053481E-05) * X + 9.25197374512647E-05) * X
//                                - 6.40994113129432E-04) * X + 3.78787044215009E-03) * X
//                        - 1.85185172458485E-02) * X + 7.14285713298222E-02) * X
//                - 1.99999999997023E-01) * X + 3.33333333333318E-01);
//    } else {
//        WW1 = (1.0 - X) / (2.0 * n + 1.0);
//    }
//
//    if (X > 1.0E-1 || (X > 1.0E-4 && n < 4)) {
//        VY(0, 0, 0) = WW1;
//        for (int m = 1; m <= n; m++) {
//	    VY(0, 0, m) = (((2.0 * m - 1.0) * VY(0, 0, m - 1)) - E) * 0.5 * XINV;
//        }
//    } else {
//        VY(0, 0, n) = WW1;
//        for (int m = n - 1; m >= 0; m--) {
//	    VY(0, 0, m) = (2.0 * X * VY(0, 0, m + 1) + E) / (m * 2.0 + 1.0);
//        }
//    }
//}


#endif
